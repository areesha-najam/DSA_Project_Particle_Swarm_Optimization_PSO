...
optimizer = ps.single.GlobalBestPSO(
    n_particles=20, dimensions=2, options=options, bounds=bounds, velocity_clamp=velocity_clamp)
# optimizing
...
for K in range(10, 510, 10):
    ...
    for i in range(0, 10):
        ...
        options = {'c1': 0.5, 'c2': 0.3, 'w': 0.9}
        optimizer = ps.single.GlobalBestPSO(
            n_particles=K, dimensions=2, options=options, bounds=bounds, velocity_clamp=velocity_clamp)
        optimizer.optimize(fx.rastrigin, 50)
        end = time.time()
...
for K in range(2, 26):
    ...
    for i in range(0, 10):
        ...
        options = {'c1': 0.5, 'c2': 0.3, 'w': 0.9}
        optimizer = ps.single.GlobalBestPSO(
            n_particles=20, dimensions=K, options=options, bounds=bounds, velocity_clamp=velocity_clamp)
        optimizer.optimize(fx.rastrigin, 50)
        end = time.time()
        runtime.append(end-start)
        # print(runtime)
